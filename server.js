...
app.get('/team_stats') {
  var query1 = 'SELECT * FROM football_game;';
  var query2 = 'SELECT COUNT(*) AS wins FROM football_game WHERE home_score > visitor_score;';
  var query3 = 'SELECT COUNT(*) AS loses FROM football_game WHERE home_score < visitor_score;';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query1),
          task.any(query2),
...
app.get('/player_info/select_player', function(req,res)) {
  var query1 = 'SELECT id, name FROM football_players;';
  var query2 = 'SELECT year, major, passing_yards, rushing_yards,receiving_yards, img_src From football_players;';
  var query3 = 'SELECT COUNT(*) AS games_played FROM football_games WHERE '+ player_select +' = ANY(players);';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query1),
          task.any(query2),
